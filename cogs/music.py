            ...
            with YoutubeDL(yt_dl_opts) as ydl:
                if self.is_url():
                    ...
                    info_dict = ydl.extract_info(
                        f"ytsearch:{self.query}", download=False
                    )
            return info_dict
...
async def play(ctx, url):
    ...
    if url == None:
        async with ctx.typing():
            await ctx.send("You must provide a valid URL, try again.")
        logger.warning(
            f"User {ctx.author.name} in {ctx.guild} attempted to call !play without providing a URL."
        )
    song = Song(url)
