...
try:
    s3 = boto3.resource(service_name='s3', region_name=getenv('AWS_REGION'),
...
async def colin(ctx):
    ...
    async with ctx.typing():
        try:
            ...
            file = File(BytesIO(s3.Object(getenv('BUCKET_NAME'), random_picture).get()['Body'].read()), filename=random_picture.split('/')[-1])
...
async def waifu(ctx):
    ...
    async with ctx.typing(): 
        ...
        try:
            ...
            logger.info(f"Random waifu has been posted successfully.")
        except Exception as e:
...
class Cog(commands.Cog, name="misc"):
    def __init__(self, bot):
        try:
            self.bot = bot
            logger.info(f"Misc cog successfully initialized.")
        except Exception as e:
    ...
    async def ninja(self, ctx):
        await ninja(ctx)
    @commands.command(name="random", help="Chosen by fair dice roll. Guaranteed to be random.")
