...
def main():
    ...
    async def thoughts(ctx, x: int=None):
    ...
    async def bj(ctx, bet: int=None):
        await gamba.blackjack(ctx, bet)
    @bot.command(name="blowjob", help="Long for bj.")
    async def blowjob(ctx, bet: int=None):
        await gamba.blackjack(ctx, bet)
    ...
    async def sluts(ctx, bet: int=None):
        await gamba.slots(ctx, bet)
    ...
    async def send(ctx, amount: int=None, recipient: str=None):
